...
- Allowlisted Discord operators (config `allowed_user_ids` / env). `/restart_maestro` stays **Luipy only**. No secrets on Discord.
- **Discord homes (dual-home):** `config.json` → `discord.homes[]` (`guild_id` + `channel_id`). NL and `/ca_persist` listen **guild-wide** (any channel in those guilds; **not DMs**) for allowlisted users. Persist threads open under **that guild's** home channel from `homes[]` (legacy fallback: `discord.thread_home_channel_id` / `DISCORD_THREAD_HOME_CHANNEL_ID` only when the guild has no home). Slash commands sync to every listed guild. Legacy single `guild_id`/`channel_id` still works. Override homes: `DISCORD_HOMES=guild:channel,guild:channel`.
...
- **Outbound files:** agent saves under `data/outbound/<thread_id>/` (or `_oneshot/…`); Maestro attaches allowed types after the turn (images, JSON, HTML, PDF, TXT/MD/CSV/LOG, SVG, XML/YAML, `.red`, ZIP, WebM). Never put secrets there.
- `/thread_end` posts a short session summary to the parent Discord channel before deleting the thread. Optional `clean:True` skips the automatic Redmine close note (Discord summary + delete still run).
...
- **Archify (diagrams):** catalog id **`archify`** on lu-zero (`/root/outils/archify`). Skill: `.cursor/skills/archify/SKILL.md`. Example map: `docs/archify/maestro-runtime.architecture.html`.
- **Redmine (MaestroBot):** API user for notes/tickets. Secrets in `.env` (`REDMINE_*`). Catalog field `redmine_doc_issue_id` (this project: **#8066**). Orphan close notes → Lost & Found **#8077**. Skill: `.cursor/skills/redmine-notes/SKILL.md`. Rule: `.cursor/rules/maestro-redmine-notes.mdc`. Notes = ASD-STE100 English inside Textile `{{collapse(Title · Project · cursor-stamp)}}` (first 8 hex chars of Cursor chat id; not issue id / not Discord HHMM). **Textile only — never Markdown in journals.** On `/thread_end`, runtime posts **one** close note: catalog hub → else one primary mentioned work ticket → else `#8077` (no fan-out). Skip with `/thread_end clean:True`. App host = amvara3; public URL via stunnel.
